...
const AIRTABLE_BASE_ID: &str = "appLMv58EvJqp11pl"; // Original base ID
const AIRTABLE_TABLE_NAME: &str = "Posts"; // Original table name
const AIRTABLE_NEW_BASE_ID: &str = "appqGKkwK7M7WIPKa"; // New base ID for "Table 1"
const AIRTABLE_NEW_TABLE_NAME: &str = "Table 1"; // New table name
...
async fn main() -> Result<(), CustomError> {
    loop {
        ...
        println!("5. Transfer data to 'Table 1' in another base");
        println!("6. Exit");
        ...
        match choice {
            ...
            5 => transfer_data_to_another_base().await?,
            6 => break,
...
async fn transfer_data_to_another_base() -> Result<(), CustomError> {
    ...
    println!("Do you want to transfer this post to 'Table 1' in another base? (yes/no)");
    ...
    println!("Transferring post to 'Table 1' in another base...");
    create_post_in_table(&post_to_transfer, AIRTABLE_NEW_BASE_ID, AIRTABLE_NEW_TABLE_NAME).await?;

    println!("Post transferred successfully.");
    Ok(())
}

async fn create_post_in_table(
    post: &Post,
    base_id: &str,
    table_name: &str,
) -> Result<(), CustomError> {
    ...
    if !response.status().is_success() {
        ...
        return Err(CustomError::Airtable(format!(
            "Failed to create post in table '{}' (base '{}'): {}",
            table_name, base_id, error_message
        )));
    }
...
async fn create_post(post: &Post) -> Result<(), CustomError> {
    create_post_in_table(post, AIRTABLE_BASE_ID, AIRTABLE_TABLE_NAME).await
}
